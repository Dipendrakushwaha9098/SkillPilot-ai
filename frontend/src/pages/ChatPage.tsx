...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    if (topic && messages.length === 1) {
      const initialMsg = `I'm studying **${topic}** and I'd like to dive deeper into it. Can you explain the core concepts and maybe give me a practical example?`;
      setInput(initialMsg);
      // We don't call send() directly here because we want the user to see/edit it or they can just hit send.
      // Or we can auto-send if we want a smoother transition. Let's auto-send.
    }
  }, [searchParams, messages]); // Added messages to dependency array to re-evaluate if messages change
